...
func TestSingleInterceptor(t *testing.T) {
	...
	if len(interceptors.Build()) == 0 {
		t.Fatalf("interceptor builder should not be empty")
	}

	_ = interceptors.StreamServerInterceptor(42, nil, nil, nullSHandler)
	_, _ = interceptors.UnaryServerInterceptor(context.Background(), 666, nil, nullUHandler)
...
func TestDoubleInterceptor(t *testing.T) {
	...
	if len(interceptors.Build()) == 0 {
		t.Fatalf("interceptor build should not be empty")
	}
