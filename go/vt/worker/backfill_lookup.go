	...
	for _, c := range chunks {
		...
		go func(chunk chunk) {
			...
			if blw.destinationAutoIncrement != nil {
				...
				queryService, err = tabletconn.GetDialer()(seqTablet, true)
				if err != nil {
			...
			transform := func(row []sqltypes.Value) ([]sqltypes.Value, error) {
				if blw.destinationAutoIncrement != nil {
					...
					results, err := queryService.Execute(ctx, target, fmt.Sprintf("select next :n values from %s", blw.destinationAutoIncrement.Sequence), bindVars, 0, nil)
					if err != nil {
						return nil, err
					}
